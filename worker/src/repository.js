...
export class D1EditorialRepository {
  ...
  async createPhoto(input) { const id=crypto.randomUUID(), timestamp=now(); await this.db.prepare('INSERT INTO photos(id,issue_id,article_submission_id,student_google_id,drive_file_id,filename,mime_type,byte_size,caption,photographer,source_type,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)').bind(id,input.issueId,input.articleSubmissionId,input.studentGoogleId,input.driveFileId,input.filename,input.mimeType,input.byteSize,input.caption,input.photographer,input.sourceType,'unreviewed',timestamp,timestamp).run(); return this.db.prepare('SELECT * FROM photos WHERE id=?').bind(id).first(); }
