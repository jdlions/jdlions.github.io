...
export function validateAssignmentDeletion(input){
  if(input?.confirmation!=='삭제'||input?.mode!=='preserve_articles')throw Object.assign(new Error('확인란에 정확히 삭제를 입력해 주세요.'),{status:400,code:'invalid_deletion_confirmation'});
}
...
function nativeForViewer(article,viewer){
  ...
  const fields=['id','issueId','studentId','articleType','titleKo','titleEn','status','draftHtml','editorDraftHtml','currentStudentRevisionId','currentEditorRevisionId','createdAt','updatedAt','submittedAt','studentFeedback','assignmentInstanceId','assignmentSlotId','campaignId','assignmentName','slotName','assignmentDueAt','slotDueAt'];
