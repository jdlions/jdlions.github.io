...
async function routeApi(request, env, pathname) {
  ...
  if(nativeArticle&&request.method==='PATCH'){requireStudent(viewer);const found=await repo.getNativeArticle(nativeArticle[1]);if(!found||found.studentId!==viewer.studentId)throw Object.assign(new Error('Article not found.'),{status:404,code:'article_not_found'});if(!editableByStudent(found))throw Object.assign(new Error('Submitted articles are locked until revision is requested.'),{status:409,code:'article_locked'});return ok(await repo.saveStudentDraft(found.id,validateNativeDraft(await request.json())),env);}
