...
test('photo gallery uses authenticated lazy-loaded image URLs',async()=>{
  ...
  for(const source of [admin,student]){assert.match(source,/loading="lazy"/);assert.match(source,/thumbnailUrl/);assert.doesNotMatch(source,/thumbnailLink/);}
...
test('admin and student apps render loading and retry states without top-level service await', async () => {
  ...
  for(const source of [admin,student]){assert.match(source,/createViewLoader/);assert.match(source,/data-retry-startup/);}
});
...
test('dataset loads coalesce, retry failures, and detail is independent of list loading',async()=>{
 let photos=0,details=0,release;
 const service=ProductionEditorialService.empty({role:'admin'},async path=>{
  if(path==='/api/photos'){photos++;if(photos===1)throw new Error('offline');return [];}
  if(path==='/api/native/articles/direct'){details++;await new Promise(r=>release=r);return {id:'direct',draftHtml:'Full text'};}
  if(path==='/api/native/articles/missing')throw new Error('not found');
  throw new Error(path);
 });
 await assert.rejects(service.ensureResource('photos'));await service.ensureResource('photos');await service.ensureResource('photos');assert.equal(photos,2);
 const first=service.getArticleDetail('direct'),second=service.getArticleDetail('direct');release();assert.deepEqual(await first,await second);assert.equal(details,1);
 await assert.rejects(service.getArticleDetail('missing'));assert(!service.listArticles().some(x=>x.id==='missing'));
});
